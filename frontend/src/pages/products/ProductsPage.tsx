...
function ProductCard({ product: p, theme, dark, onEdit, onDelete, onAdjust }: {
  ...
  return (
    <div
      ...
      style={{ background: hov ? theme.cardBgHover : theme.cardBg, border: `1px solid ${hov ? PALETTE.primary + "44" : p.stock_status !== "ok" ? (p.stock_status === "rupture" ? "#ef444444" : "#f59e0b44") : theme.border}`, borderRadius: "14px", overflow: "hidden", transition: "all 0.2s", transform: hov ? "translateY(-2px)" : "none", boxShadow: hov ? (dark ? "0 8px 28px rgba(0,0,0,0.35)" : "0 6px 20px rgba(6,182,212,0.1)") : "none" }}
    >
      {/* Image */}
...
export default function ProductsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Filtre stock côté client (le backend ne filtre pas par stock_status)
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.75rem" }}>
        <div>
          ...
          <p style={{ fontSize: "0.85rem", color: theme.textMuted, marginTop: 3 }}>
            Gérez votre catalogue produits et suivez les niveaux de stock
          </p>
        </div>
      ...
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1rem", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: "0.35rem" }}>
          {(["", "ok", "faible", "rupture"] as const).map(f => {
            const cfg = f === "" ? { label: "Tous", color: theme.textMuted } : { label: STOCK_STATUS[f].label, color: STOCK_STATUS[f].text };
            return (
              <button key={f} onClick={() => setStockFilter(f)}
                style={{ padding: "0.45rem 0.85rem", borderRadius: RADIUS.md, border: `1px solid ${stockFilter === f ? (f === "" ? PALETTE.primary : STOCK_STATUS[f as keyof typeof STOCK_STATUS]?.text ?? PALETTE.primary) : theme.border}`, background: stockFilter === f ? (f === "" ? `linear-gradient(135deg,${PALETTE.primary},${PALETTE.primaryEnd})` : STOCK_STATUS[f as keyof typeof STOCK_STATUS]?.bg ?? "transparent") : "transparent", color: stockFilter === f ? (f === "" ? "#fff" : cfg.color) : theme.textMuted, fontSize: "0.78rem", cursor: "pointer", fontFamily: FONTS.body, transition: "all 0.15s" }}>
                {cfg.label}
              </button>
            );
          })}
