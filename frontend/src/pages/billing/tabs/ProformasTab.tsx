// ─────────────────────────────────────────────────────────────────────────────
//  src/pages/billing/tabs/ProformasTab.tsx
// ─────────────────────────────────────────────────────────────────────────────
...
import ProformaFormModal   from "../../proformas/components/ProformaFormModal";
import ProformaDetailModal from "../../proformas/components/ProformaDetailModal";
...
function StatCard({ label, value, color, icon, sub, onClick, theme }: {
  ...
  return (
    <div className="pf-sc-card" onClick={onClick}
      ...
      <div className="pf-sc-icon" style={{ width: 42, height: 42, borderRadius: "11px", background: color + "18", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", flexShrink: 0 }}>
        {icon}
      </div>
      <div style={{ minWidth: 0, flex: 1 }}>
        <div style={{ fontSize: "0.68rem", color: theme.textMuted, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {label}
        </div>
        <div className="pf-sc-val" style={{ fontSize: "1.3rem", fontWeight: 700, fontFamily: FONTS.display, color: theme.textPrimary, lineHeight: 1 }}>
          {value}
        </div>
...
const CSS = `
  ...
  @media (min-width: 400px) {
    .pf-stats { grid-template-columns: repeat(2, 1fr); gap: 0.65rem; }
  }
  @media (min-width: 640px) {
    .pf-stats { grid-template-columns: repeat(4, 1fr); gap: 0.85rem; margin-bottom: 1.5rem; }
  }

  /* ── StatCard responsive ── */
  @media (max-width: 399px) {
    /* 1 col : carte horizontale large, valeur toujours lisible */
    .pf-sc-card { padding: 0.75rem 1rem !important; }
    .pf-sc-icon { width: 38px !important; height: 38px !important; font-size: 1.1rem !important; border-radius: 10px !important; }
    .pf-sc-val  { font-size: 1.2rem !important; }
  }
  @media (min-width: 400px) and (max-width: 639px) {
    /* 2 col : compact mais lisible */
    .pf-sc-card { padding: 0.7rem 0.75rem !important; gap: 0.5rem !important; }
    .pf-sc-icon { width: 34px !important; height: 34px !important; font-size: 1rem !important; border-radius: 9px !important; }
    .pf-sc-val  { font-size: 1rem !important; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  }
...
// ── ProformasTab ──────────────────────────────────────────────────────────────
export default function ProformasTab({ dark, theme }: TabProps) {
  const swal = useSwal();
  ...
  const handleDuplicate = async (p: Proforma) => {
  ...
  const proformasMoisCourant = allProformas.filter(p => isThisMonth(p.date));

  const montantMoisCourant = proformasMoisCourant
    .reduce((sum, p) => sum + p.total_ttc, 0);

  const montantEnAttenteMoisCourant = proformasMoisCourant
    .filter(p => p.status in ["draft", "sent"] && !p.converted_to_invoice)
    .reduce((sum, p) => sum + p.total_ttc, 0);
  ...
  return (
    <div style={{ fontFamily: FONTS.body }}>
      ...
      <div className="pf-header">
        <div>
          <h2 style={{ fontFamily: FONTS.display, fontSize: "1.15rem", fontWeight: 700, color: theme.textPrimary, letterSpacing: "-0.01em" }}>
            Proformas
          </h2>
          <p style={{ fontSize: "0.8rem", color: theme.textMuted, marginTop: 2 }}>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.6rem 1.1rem", borderRadius: RADIUS.md, border: "none", background: "linear-gradient(135deg,#8b5cf6,#6d28d9)", color: "#fff", fontSize: "0.875rem", fontWeight: 600, cursor: "pointer", fontFamily: FONTS.body, boxShadow: "0 0 20px rgba(139,92,246,0.22)", whiteSpace: "nowrap" }}>
          + Nouveau proforma
        </button>
      </div>
      ...
      {dashboard && (
        <div className="pf-stats">
          {/* Montant total du mois courant */}
          <StatCard
            label="Montant ce mois"
            value={`${fmt(montantMoisCourant)} F`}
            color="#8b5cf6" icon="💜"
            sub={`${monthLabel} · ${proformasMoisCourant.length} proforma${proformasMoisCourant.length > 1 ? "s" : ""}`}
            theme={theme}
          />

          {/* Nombre de proformas ce mois */}
          <StatCard
            label="Ce mois"
            value={proformasMoisCourant.length}
            color="#06b6d4" icon="📅"
            sub={monthLabel}
            theme={theme}
          />
          ...
          {/* Montant en attente du mois courant */}
          <StatCard
            label="En attente ce mois"
            value={`${fmt(montantEnAttenteMoisCourant)} F`}
            color={PALETTE.danger} icon="⏳"
            sub={`Non convertis — ${monthLabel}`}
            theme={theme}
          />
        </div>
      )}
      ...
      {loading ? (
        <div style={{ display: "flex", justifyContent: "center", padding: "3rem" }}>
          <div style={{ width: 34, height: 34, border: "3px solid rgba(139,92,246,0.25)", borderTopColor: "#8b5cf6", borderRadius: "50%", animation: "pf-spin 0.8s linear infinite" }} />
        </div>
      ) : proformas.length === 0 ? (
                          ...
                          <div style={{ display: "flex", gap: "0.25rem" }}>
                            {[
                              ...
                              ...(!p.converted_to_invoice ? [
                                ...
                                { icon: "📋", color: "#06b6d4", title: "Dupliquer", action: () => handleDuplicate(p) },
                              ] : []),
            ...
            {proformas.map(p => {
              ...
              const actions = [
                ...
                ...(!p.converted_to_invoice ? [
                  ...
                  { icon: "📋", label: "Dupliquer", color: "#06b6d4", action: () => handleDuplicate(p) },
                ] : []),
