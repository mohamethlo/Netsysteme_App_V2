...
import { billingService, type Invoice, type BillingClient, type InvoiceItemPayload } from "../../../services/billingService";
import { pdfService } from "../../../services/pdfService";
import { SearchableSelect } from "../../../components/SearchableSelect";
import { billingClientsService } from "../../../services/billingClientsService";
...
interface Props {
  ...
  products: { id: number; name: string | null; description: string | null; price: number; quantity: number; is_low_stock: boolean }[];
  onClose: () => void; onSaved: (created?: Invoice) => void;
}

interface LineItem {
  description: string; quantity: number;
  unit_price: number; discount_percent: number; product: number | null;
}

const emptyLine = (): LineItem => ({ description: "", quantity: 1, unit_price: 0, discount_percent: 0, product: null });
...
type TabKey = "invoice" | "client" | "product";
...
const CSS = `
  ...
  @media (min-width: 640px) {
    .ifm-overlay { align-items: center; padding: 0.5rem; }
  }
  ...
  @media (min-width: 640px) {
    .ifm-modal { max-width: 880px; border-radius: 18px; max-height: 96vh; }
  }
  ...
  .ifm-tabs::-webkit-scrollbar { display: none; }
  ...
  @media (min-width: 640px) {
    .ifm-tab-btn { padding: 0.8rem 1.25rem; font-size: 0.875rem; }
  }

  .ifm-body { padding: 1rem; }
  @media (min-width: 640px) { .ifm-body { padding: 1.5rem; } }
  ...
  @media (min-width: 480px) { .ifm-grid-info { grid-template-columns: 1fr 1fr; } }
  @media (min-width: 768px) { .ifm-grid-info { grid-template-columns: repeat(3, 1fr); } }
  ...
  @media (min-width: 640px) { .ifm-grid-recap { grid-template-columns: 1fr 1fr; } }
  ...
  @media (min-width: 480px) {
    .ifm-footer { flex-direction: row; justify-content: flex-end; }
  }
  ...
  @media (min-width: 480px) { .ifm-inner-grid2 { grid-template-columns: 1fr 1fr; } }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
      <div style={{ padding: "0.75rem 1rem", borderRadius: RADIUS.md, background: `rgba(6,182,212,0.06)`, border: `1px solid rgba(6,182,212,0.2)`, fontSize: "0.82rem", color: theme.textSecondary }}>
        💡 Le client sera automatiquement sélectionné dans la facture après création.
      </div>

      <div className="ifm-inner-grid2">
        <div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", paddingTop: "0.5rem", borderTop: `1px solid ${theme.border}` }}>
        <button type="submit" disabled={saving}
          ...
          {saving
            ? <><span style={{ width: 13, height: 13, border: "2px solid rgba(255,255,255,0.35)", borderTopColor: "#fff", borderRadius: "50%", animation: "ifm-spin 0.7s linear infinite", display: "inline-block" }} /> Création…</>
            : "✓ Créer le client"}
        </button>
      </div>
    </form>
...
function InlineProductForm({ dark, theme, onCreated }: {
  dark: boolean; theme: LayoutContext["theme"];
  onCreated: (product: { id: number; name: string | null; description: string | null; price: number; quantity: number; is_low_stock: boolean }) => void;
}) {
  const swal    = useSwal();
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
      <div style={{ padding: "0.75rem 1rem", borderRadius: RADIUS.md, background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.2)", fontSize: "0.82rem", color: theme.textSecondary }}>
        💡 Le produit sera automatiquement disponible dans la liste des articles de la facture.
      </div>
...
// ── InvoiceFormModal ──────────────────────────────────────────────────────────
export default function InvoiceFormModal({ dark, theme, invoice, clients: initialClients, products: initialProducts, onClose, onSaved }: Props) {
  const swal   = useSwal();
  const isEdit = !!invoice;
  const [saving,    setSaving]    = useState(false);
  const [activeTab, setActiveTab] = useState<TabKey>("invoice");

  // Listes locales enrichissables sans recharger le parent
  ...
  const [form, setForm] = useState({
    ...
    billing_client:  invoice?.billing_client?.toString() ?? "",
    date:            invoice?.date            ?? new Date().toISOString().split("T")[0],
    due_date:        invoice?.due_date        ?? "",
    tax_rate:        invoice?.tax_rate?.toString() ?? "0.18",
    domaine:         invoice?.domaine         ?? "" as "" | "NETSYSTEME" | "SSE",
    notes:           invoice?.notes           ?? "",
    discount_type:   invoice?.discount_percent ? "percent" : invoice?.discount_amount ? "amount" : "none",
    discount_value:  (invoice?.discount_percent ?? invoice?.discount_amount ?? 0).toString(),
    advance_amount:  invoice?.advance_amount?.toString() ?? "0",
  });

  const [lines, setLines] = useState<LineItem[]>(
    invoice?.items?.length
      ? invoice.items.map(it => ({ description: it.description ?? "", quantity: it.quantity, unit_price: it.unit_price, discount_percent: it.discount_percent, product: it.product }))
      : [emptyLine()]
  );

  useEffect(() => {
    if (!isEdit && !form.invoice_number) {
      billingService.invoices.getNextNumber()
        .then(r => setForm(f => ({ ...f, invoice_number: r.number })))
        .catch(() => {});
    }
  }, []);
  ...
  const selectProduct = (idx: number, productId: string) => {
    ...
    if (p) setLines(ls => ls.map((l, i) => i === idx ? { ...l, product: p.id, description: p.name ?? p.description ?? "", unit_price: p.price } : l));
    else   setLine(idx, "product", null);
  };
  ...
  const handleProductCreated = (product: typeof localProducts[0]) => {
  ...
  const discVal   = form.discount_type === "percent" ? totalTTC * (parseFloat(form.discount_value) / 100) : form.discount_type === "amount" ? parseFloat(form.discount_value) || 0 : 0;
  const netTTC    = totalTTC - discVal;
  const advance   = parseFloat(form.advance_amount) || 0;
  const remaining = Math.max(0, netTTC - advance);
  const fmt = (n: number) => new Intl.NumberFormat("fr-FR").format(Math.round(n));

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const validLines = lines.filter(l => l.description.trim());
    if (!form.billing_client) { swal.error("Requis", "Sélectionnez un client.");      return; }
    if (!form.domaine)         { swal.error("Requis", "Sélectionnez un domaine.");     return; }
    if (!validLines.length)    { swal.error("Requis", "Ajoutez au moins un article."); return; }
    ...
    try {
      const payload = {
        ...
        items:            validLines as InvoiceItemPayload[],
      };
      if (isEdit) {
        await billingService.invoices.update(invoice!.id, payload);
        swal.updated("La facture");
        if (pdfTab) pdfTab.close();
        onSaved();
      } else {
        const created    = await billingService.invoices.create(payload);
        const clientName = localClients.find(c => c.id === parseInt(form.billing_client))?.display_name;
        swal.saved("La facture");
        if (pdfTab) pdfService.invoice.viewInTab(pdfTab, created.id, clientName, created.invoice_number);
        onSaved(created);
      }
    } catch (err: any) {
      const d = err?.response?.data;
      const msg = d?.detail ?? d?.invoice_number?.[0] ?? d?.billing_client?.[0] ?? d?.domaine?.[0] ?? d?.items?.[0] ?? "Une erreur est survenue.";
  ...
  const TABS: { key: TabKey; label: string; icon: string; color: string }[] = [
    { key: "invoice", label: "Facture",          icon: "📄", color: PALETTE.primary },
    { key: "client",  label: "+ Nouveau client", icon: "👤", color: "#06b6d4"       },
  ...
  return (
    <>
      ...
      <div className="ifm-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="ifm-modal" style={{ background: theme.popupBg, border: `1px solid ${theme.border}`, boxShadow: dark ? "0 32px 80px rgba(0,0,0,0.65)" : "0 24px 60px rgba(0,0,0,0.15)", fontFamily: FONTS.body }}>
          ...
          <div style={{ position: "sticky", top: 0, background: theme.popupBg, zIndex: 10, borderRadius: "18px 18px 0 0", borderBottom: `1px solid ${theme.border}` }}>
            <div style={{ padding: "1rem 1.25rem 0.75rem", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <p style={{ fontSize: "0.75rem", color: theme.textMuted }}>
                  {activeTab === "invoice"
                    ? (isEdit ? "Modifiez les informations de la facture" : "Renseignez les informations")
                    : activeTab === "client"
            ...
            {/* Onglets */}
            <div className="ifm-tabs">
              {TABS.map(tab => (
                <button key={tab.key} type="button" className="ifm-tab-btn"
            ...
            <form onSubmit={handleSubmit}>
              <div className="ifm-body">
                ...
                <div style={{ marginBottom: "1.25rem" }}>
                  ...
                  <div className="ifm-grid-info">
                    <div>
                      <label style={lbl}>Numéro *</label>
                      <input required value={form.invoice_number} onChange={e => setF("invoice_number", e.target.value)} style={inp} />
                    </div>
                    <div>
                      ...
                      <div style={{ display: "flex", gap: "0.4rem" }}>
                        ...
                        <button type="button" onClick={() => setActiveTab("client")} title="Créer un client"
                          style={{ flexShrink: 0, width: 36, height: 36, borderRadius: RADIUS.md, border: `1px solid rgba(6,182,212,0.4)`, background: "transparent", color: "#06b6d4", cursor: "pointer", fontSize: "1rem", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          +
                        </button>
                      </div>
                    </div>
                        ...
                        <div key={i} style={{ border: `1px solid ${theme.border}`, borderRadius: RADIUS.lg, overflow: "hidden", background: dark ? "rgba(255,255,255,0.015)" : "rgba(0,0,0,0.01)" }}>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.5rem 0.85rem", background: dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.03)", borderBottom: `1px solid ${theme.border}` }}>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                              <span style={{ fontSize: "0.85rem", fontWeight: 700, color: PALETTE.primary }}>{fmt(lineTotal)} F</span>
                              <button type="button" onClick={() => setLines(ls => ls.filter((_, idx) => idx !== i))} disabled={lines.length === 1}
                                style={{ width: 26, height: 26, borderRadius: "7px", border: "1px solid rgba(239,68,68,0.3)", background: "transparent", color: "#ef4444", cursor: lines.length === 1 ? "not-allowed" : "pointer", opacity: lines.length === 1 ? 0.3 : 1, fontSize: "0.8rem", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
                            </div>
                          </div>
                          {/* Champs */}
                          <div style={{ padding: "0.75rem 0.85rem", display: "flex", flexDirection: "column", gap: "0.55rem" }}>
                                ...
                                <SearchableSelect
                                  options={[
                                    ...
                                    ...localProducts.map(p => ({
                                      ...
                                      sub: `${new Intl.NumberFormat("fr-FR").format(Math.round(p.price))} F`,
                                    })),
                                  ]}
                            ...
                            <div>
                              <label style={lbl}>Description *</label>
                              <textarea value={line.description} onChange={e => setLine(i, "description", e.target.value)} placeholder="Description de l'article…" rows={2} style={{ ...inp, resize: "vertical", minHeight: 60 }} />
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.55rem" }}>
                              <div>
                                <label style={lbl}>Quantité</label>
                                <input type="number" min="0.01" step="0.01" value={line.quantity} onChange={e => setLine(i, "quantity", parseFloat(e.target.value) || 0)} style={inp} />
                              </div>
                              <div>
                                <label style={lbl}>Prix unitaire (F)</label>
                                <input type="number" min="0" step="0.01" value={line.unit_price} onChange={e => setLine(i, "unit_price", parseFloat(e.target.value) || 0)} style={inp} />
                              </div>
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.55rem", alignItems: "end" }}>
                              <div>
                                <label style={lbl}>Remise (%)</label>
                                <input type="number" min="0" max="100" step="0.01" value={line.discount_percent} onChange={e => setLine(i, "discount_percent", parseFloat(e.target.value) || 0)} style={inp} />
                              </div>
                ...
                <div className="ifm-grid-recap">
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    ...
                    {form.discount_type !== "none" && (
                      <div>
                        <label style={lbl}>{form.discount_type === "percent" ? "Remise (%)" : "Remise (F)"}</label>
                        <input type="number" min="0" step="0.01" value={form.discount_value} onChange={e => setF("discount_value", e.target.value)} style={inp} />
                      </div>
                    )}
                    <div>
                      <label style={lbl}>Avance versée (F)</label>
                      <input type="number" min="0" step="0.01" value={form.advance_amount} onChange={e => setF("advance_amount", e.target.value)} style={inp} />
                    </div>
                    <div>
                      <label style={lbl}>Notes</label>
                      <textarea value={form.notes} onChange={e => setF("notes", e.target.value)} rows={3} style={{ ...inp, resize: "vertical", minHeight: 76 }} />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ background: dark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.025)", borderRadius: RADIUS.lg, border: `1px solid ${theme.border}`, overflow: "hidden" }}>
                      {[
                        ...
                        ...(advance  > 0 ? [{ label: "Avance", value: -advance, bold: false }] : []),
                      ].map((row, i, arr) => (
                      ...
                      <div style={{ padding: "0.85rem 1rem", background: dark ? "rgba(6,182,212,0.08)" : "rgba(6,182,212,0.05)", borderTop: `1px solid ${theme.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "0.875rem", fontWeight: 700, color: theme.textPrimary }}>Reste à payer</span>
                        <span style={{ fontSize: "1.25rem", fontWeight: 800, fontFamily: FONTS.display, color: PALETTE.primary }}>{fmt(remaining)} F</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Footer */}
              <div className="ifm-footer" style={{ borderTopColor: theme.border, padding: "1rem" }}>
                <button type="submit" disabled={saving}
                  ...
                  {saving
                    ? <><span style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.35)", borderTopColor: "#fff", borderRadius: "50%", animation: "ifm-spin 0.7s linear infinite", display: "inline-block" }} /> Enregistrement…</>
